...
import { scoreToBackgroundColor, scoreToTextColor, formatScoreWithDelta } from '@/utils/scoring';
    ...
    >
      <div className="space-y-3">
          ...
          {[attacker1, attacker2].map((attacker) => {
            ...
            return (
              ...
              >
                ...
                <div className="w-full truncate text-xs text-gray-500">{attacker.faction}</div>
                {isForced && (
                  <div className="mt-1 text-xs font-medium text-blue-600">Will play</div>
                )}
              </div>
            );
          })}
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="truncate text-sm text-gray-500">vs {oppDefender.faction}</span>
        </div>
      </div>
    </Card>
  );
